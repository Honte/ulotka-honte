...
\mysec{Zasada 1}
Na początku gry plansza jest \myem{pusta}. Czarny wykonuje pierwszy ruch. Gracze wykonują ruchy \myem{na zmianę}. Ruch polega na postawieniu kamienia na pustym \myem{przecięciu} linii. Na~brzegu oraz w narożniku również można stawiać kamienie. Raz postawiony kamień nie zmienia położenia.
...
\mysec{Zasada 6}
Po zakończeniu gry wszystkie \myem{martwe} kamienie usuwa się z~planszy i~dodaje do jeńców (patrz też dodatek 2). Każdy z~graczy otrzymuje po jednym \myem{punkcie} za każde puste \myem{przecięcie} wewnątrz swojego terytorium (obszaru otoczonego swoimi kamieniami). Do tego dodaje się liczbę \myem{jeńców}. Biały dostaje dodatkowo punkty \myem{komi} jako rekompensatę, za to że rozpoczynał grę jako drugi. Obecnie komi wynosi \myem{6,5} punktu. Ten, kto zdobył \myem{więcej} punktów wygrywa.
\vskip\parskip
...
Inną sytuację widać na kolejnych rysunkach. Biały \myem{nie jest} w stanie uratować tych kamieni. Ma wprawdzie 2~oddechy, ale czarny może tam zagrać, \myem{odbierając} mu oba.
\vskip\parskip
\hbox to \hsize{%
...
Oto jeszcze jeden przykład skończonej gry. Czy wiadomo dlaczego kamienie zaznaczone są martwe?
...
W go ważna jest \myem{różnica} między punktami przeciwników. Zamiast dodawać \myem{jeńców} do punktów można więc \myem{odjąć} ich liczbę od punktów przeciwnika -- wkładając jeńców w jego teren. Oczywiście nie zmienia to wyniku, za to ułatwia liczenie.
\vskip\parskip
\hbox to \hsize{\includegraphics{diag/d2-3}\hfil\includegraphics{diag/d2-4}}

Czarne mają \myem{15} punktów (\textstone{\gooegb4}).

Białe mają 9 + 6,5 = \myem{15,5} punktów (\textstone{\gooegb3}).
...
\myurl{poznan.go.art.pl}\vskip 5em 
}}

\centerline{Więcej informacji na stronach internetowych:}

{\parindent=2.5cm
\myurl{go.art.pl}

\myurl{wiki.go.art.pl}
